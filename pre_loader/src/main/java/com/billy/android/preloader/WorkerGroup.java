...
class WorkerGroup implements IWorker {
    ...
    public GroupedDataLoader getDataLoader(String keyIngroup){
        for (Worker worker : workers) {
            GroupedDataLoader dataLoader = (GroupedDataLoader) worker.dataLoader;
            if(TextUtils.equals(keyIngroup,dataLoader.keyInGroup())){
                return (GroupedDataLoader) worker.dataLoader;
            }
        }
        return null;
    }
    ...
    public boolean refresh(String key) {
        boolean success = true;
        for (Worker worker : workers) {
            if (worker.dataLoader instanceof GroupedDataLoader) {
                GroupedDataLoader dataLoader = (GroupedDataLoader) worker.dataLoader;
                if (TextUtils.equals(key, dataLoader.keyInGroup())) {
                    success &= worker.refresh();
                }
            }
        }
